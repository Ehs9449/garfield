...
class GarfieldOrthoProjector:
    ...
    def run_pipeline(self, pointcloud_path, scale, output_dir,
        ...
        # Two oblique elevation rings around the building.
        for elevation_deg in [30, 55]:
            elevation = np.radians(elevation_deg)

            for azimuth_deg in range(0, 360, 45):
                azimuth = np.radians(azimuth_deg)

                # Place camera plane around and above the building.
                horizontal_dist = max_extent * 0.9
                ...
                # Construct stable orthographic camera axes.
                ...
                # Size the orthographic image from the projected crop box.
                relative = bbox_corners - plane_center
                projected_right = relative @ right
                projected_up = relative @ up
                ...
                views.append({
                    'name': f'oblique_{elevation_deg:02d}_{azimuth_deg:03d}',
                    'plane_center': plane_center.tolist(),
                    'look_dir': look_dir.tolist(),
                    'up_vec': up.tolist(),
                    'view_width': float(view_width),
        ...
        # One true top-down orthographic view.
        top_center = np.array(
