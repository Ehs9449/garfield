...
def find_candidate_intersections(planes):
    """
    Find candidate neighboring plane pairs whose finite rectangles
    overlap along their mathematical plane-plane intersection line.
    """
    candidates = []
    ...
    for i in range(len(planes)):
        for j in range(i + 1, len(planes)):
            ...
            interval_i = rectangle_line_interval(
                planes[i]["boundary"]["corners"],
                point,
                direction,
            )

            interval_j = rectangle_line_interval(
                planes[j]["boundary"]["corners"],
                point,
                direction,
            )
            ...
            t_start = max(interval_i[0], interval_j[0])
            t_end = min(interval_i[1], interval_j[1])
            ...
            # Ignore extremely tiny accidental intersections.
            if length < 0.03:
                continue
